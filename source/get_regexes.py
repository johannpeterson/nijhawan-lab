...
parser = argparse.ArgumentParser(description='Generate regular expressions for the forward & reverse primer sequences.')
...
def main():
    ...
    forward_primer_sequences = [primer_lookup[K]['sequence'] for K in primer_lookup if primer_lookup[K]['direction']=='F']
    reverse_primer_sequences = [primer_lookup[K]['sequence'] for K in primer_lookup if primer_lookup[K]['direction']=='R']

    patterns={}
    patterns['SEQ_FWD'] = sequence_to_regex(common_sequence(forward_primer_sequences))
    patterns['SEQ_REV'] = sequence_to_regex(common_sequence(reverse_primer_sequences))
    patterns['SEQ_FWD_RC'] = sequence_to_regex(rcDNA(common_sequence(forward_primer_sequences)))
    patterns['SEQ_REV_RC'] = sequence_to_regex(rcDNA(common_sequence(reverse_primer_sequences)))
