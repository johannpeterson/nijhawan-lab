...
while(my $line = <$reader>) {
	...
	if(@tokenListList && $tokenListList[0]->[0] ne $tokenList[0]) {
		$barcodePairSequenceCountHash{$_} += 1 foreach(getBarcodePairSequenceList(@tokenListList));
		@tokenListList = ();
	}
...
foreach my $barcodePairSequence (sort {$barcodePairSequenceCountHash{$b} <=> $barcodePairSequenceCountHash{$a} || $a cmp $b} keys %barcodePairSequenceCountHash) {
	print join("\t", $barcodePairSequence, $barcodePairSequenceCountHash{$barcodePairSequence}), "\n";
}
