# jpbio/util.py
"""Utility functions for bioinformatics scripts."""
...
def rcDNA(seq):
    """Return the reverse DNA complement."""
...
def pad_list(item_list, length, element):
    """Return item_list extended to be at least length long."""
...
def pad_dict_list(item_list, length):
    """Return of list of at least length dict objects.

    Extend item_list, if necessary, by creating new empty dicts.

    It is necessary to create lists of dictionaries differently
    from lists of integers, because if you use the strategy above
    you get a list where every element is the same dict.
    """
    if len(item_list) < length:
...
def export_figure(fig, title, data_directory, experiment, metadata={}):
    """Export a figure to PNG file using a standardized file name."""
...
def common_sequence(seqs):
    ...
    return "".join(
        list(map(elements_equal, [[seq[i] for seq in seqs] for i in range(N)]))
    )
