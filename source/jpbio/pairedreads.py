...
class pairAnalyzer:
    ...
    def findBarcode(self, read, rc=False):
        if rc:
            pre_loc = read.seq.find( rcDNA(reference_sequence_pre) ) 
            post_loc = read.seq.find( rcDNA(reference_sequence_post) ) 
            barcode_loc = [post_loc + len(reference_sequence_post), pre_loc]
            ...
            barcode_loc = [pre_loc + len(reference_sequence_pre), post_loc]
    ...
    def analyzeReads(self, read1, read2):
        primers1 = self.primers.idPrimers(read1)
        primers2 = self.primers.idPrimers(read2)
